...
from FFbot.__main__ import DATA_IMPORT
...
def import_data(bot: Bot, update):
    ...
    if msg.reply_to_message and msg.reply_to_message.document:
        try:
            ...
            msg.reply_text(tld(chat.id, "Try downloading and reuploading the file as yourself before importing - this one seems "
                           "to be iffy!"))
            return
        ...
        if len(data) > 1 and str(chat.id) not in data:
            msg.reply_text(tld(chat.id, "Theres more than one group here in this file, and none have the same chat id as this group "
        ...
        try:
            ...
            msg.reply_text(tld(chat.id, "An exception occured while restoring your data. The process may not be complete. If "
        ...
        msg.reply_text(tld(chat.id, "Backup fully imported. Welcome back! :D"))
